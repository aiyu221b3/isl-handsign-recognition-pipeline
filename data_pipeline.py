...
for train_dir in KAGGLE_INPUT.rglob("train"):
    ...
    if (parent / "valid").is_dir() and (parent / "test").is_dir():
...
if len(candidates) == 0:
    ...
    for path in KAGGLE_INPUT.rglob("*"):
        if path.is_dir():
            print(path)
    raise FileNotFoundError("\nCould not find a directory containing train/, valid/, and test/.")
...
for split in SPLITS:
    split_dir = DATASET_ROOT / split
    images = [p for p in split_dir.iterdir() if p.is_file() and p.suffix.lower() in IMAGE_EXTENSIONS]
    print(f"{split:>5}: {len(images):4d} images")
    for p in images[:5]:
        print("       ", p.name)
    print()
...
def filename_to_class(filename: str) -> str:
    ...
    if normalized.startswith("yes"):
        return "Yes"
    raise ValueError(f"Could not infer class from filename: {filename}")
...
for split in ["train", "valid", "test"]:
    ...
    for img_path in split_dir.iterdir():
        if img_path.suffix.lower() not in IMAGE_EXTENSIONS:
            continue
        rows.append({"split": split, "filename": img_path.name, "path": str(img_path), "class": filename_to_class(img_path.name)})
...
train_df, val_df = train_test_split(dev, test_size=0.10, stratify=dev["class"], random_state=42)
