...
if not TEST_FEATURE_PATH.exists():
    raise FileNotFoundError(
        f"Test feature dataset not found:\n"
        f"{TEST_FEATURE_PATH}"
    )
...
X_node_test = torch.from_numpy(
    test_data["X_node"]
).float()

X_global_test = torch.from_numpy(
    test_data["X_global"]
).float()
...
print(
    f"Node features:    {X_node_test.shape}"
)

print(
    f"Global features:  {X_global_test.shape}"
)
...
print(
    f"Correct:        "
    f"{int(np.sum(predictions_np == y_test))}"
    f" / {len(y_test)}"
)
...
for class_id, class_name in enumerate(
    ...
    print(
        f"{class_name:10s} | "
        f"precision={test_precision[class_id]:.4f} | "
        f"recall={test_recall[class_id]:.4f} | "
        f"F1={test_f1_per_class[class_id]:.4f}"
    )
...
benchmark_node = X_node_test[:1].to(
    DEVICE
)

benchmark_global = X_global_test[:1].to(
    DEVICE
)
...
print(
    f"Latency: {model_latency_ms:.4f} ms/image"
)

print(
    f"Throughput: {model_fps:.1f} FPS"
)
...
final_metrics = {
    ...
    "per_class": {
        CLASS_NAMES[class_id]: {
            "precision": float(
                test_precision[class_id]
            ),
            "recall": float(
                test_recall[class_id]
            ),
            "f1": float(
                test_f1_per_class[class_id]
            ),
        }
...
if len(mistakes_df) == 0:
    ...
    print(
        "✅ No test-set classification errors."
    )
