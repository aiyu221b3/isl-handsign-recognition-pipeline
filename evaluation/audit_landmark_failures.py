...
for split in ["train", "valid", "test"]:
    ...
    for ann in coco["annotations"]:
        annotations_lookup.setdefault(
            ann["image_id"], []
        ).append(ann)

    coco_lookup[split] = {
        "images": image_lookup,
        "annotations": annotations_lookup,
    }
...
for _, row in failed_df.iterrows():
    ...
    image_record = coco_lookup[split]["images"].get(
        filename
    )

    if image_record is None:

        print(f"\n⚠️ No COCO image record for: {filename}")
        continue

    image_id = image_record["id"]

    annotations = coco_lookup[split]["annotations"].get(
        image_id,
        []
    )
    ...
    plt.title(
        f"{row['class']} | MediaPipe: no_hand_detected"
    )
    plt.axis("off")
    plt.show()
