...
class ISLFeatureDataset(Dataset):
    """
    Dataset backed by one .npz feature file.
    """
    ...
    def __init__(self, feature_path):
        ...
        self.X_node = torch.from_numpy(
            data["X_node"]
        ).float()

        self.X_global = torch.from_numpy(
            data["X_global"]
        ).float()
...
train_dataset = ISLFeatureDataset(
    TRAIN_PATH
)

val_dataset = ISLFeatureDataset(
    VAL_PATH
)
...
final_accuracy = final_val_metrics[
    "accuracy"
]

final_macro_f1 = final_val_metrics[
    "macro_f1"
]

final_precision = final_val_metrics[
    "precision"
]

final_recall = final_val_metrics[
    "recall"
]

final_cm = final_val_metrics[
    "confusion_matrix"
]
...
print(
    f"Final val accuracy:   {final_accuracy:.4f}"
)

print(
    f"Final val macro F1:   {final_macro_f1:.4f}"
)
...
for class_id, class_name in enumerate(
    ...
    print(
        f"{class_name:10s} | "
        f"precision={final_precision[class_id]:.4f} | "
        f"recall={final_recall[class_id]:.4f} | "
        f"F1={2 * final_precision[class_id] * final_recall[class_id] / max(final_precision[class_id] + final_recall[class_id], 1e-12):.4f}"
    )
...
ax.set_ylabel("Score")
ax.set_title(
    "ISL Static Prototype — Validation Metrics"
)
