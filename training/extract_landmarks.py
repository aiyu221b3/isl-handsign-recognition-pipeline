from isl_static_prototype.data_pipeline import DATASET_ROOT, PROJECT_ROOT, df, train_df, val_df, hand_landmarker
...
for source_split in ['train', 'valid', 'test']:
    ...
    coco_lookup[source_split] = {'images': image_lookup, 'annotations': annotation_lookup}
split_tables = {'train': train_df.copy(), 'val': val_df.copy(), 'test': df[df['split'] == 'test'].copy()}
print('Extraction dataset:')
for split, table in split_tables.items():
    print(f'  {split:>5}: {len(table):4d} images')
print()

def get_original_coco_split(split: str) -> str:
    """ fix back to original style """
    return split
...
def find_coco_annotation(image_path: Path, split: str):
    ...
    if original_split not in coco_lookup:
        return None
    image_record = coco_lookup[original_split]['images'].get(image_path.name)
    if image_record is None:
        return None
    annotations = coco_lookup[original_split]['annotations'].get(image_record['id'], [])
    if not annotations:
        return None
...
def extract_landmarks(image_path: Path):
    try:
        ...
        y2 = min(image_height, int(np.ceil(y + h * (1.0 + padding))))
        if x2 <= x1 or y2 <= y1:
        ...
        original_landmarks[:, 0] = (x1 + crop_landmarks[:, 0] * crop_width) / image_width
        original_landmarks[:, 1] = (y1 + crop_landmarks[:, 1] * crop_height) / image_height
        original_landmarks[:, 2] *= crop_width / image_width
        handedness = None
        handedness_score = None
        if crop_result.handedness and len(crop_result.handedness[0]) > 0:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if landmarks is not None:
            ...
            if metadata['extraction_method'] == 'full_image':
                ...
                crop_count += 1
            stored_path = str(landmark_path.relative_to(PROJECT_ROOT))
        else:
            failure_count += 1
            stored_path = None
        records.append({'split': split, 'filename': filename, 'class': row['class'], 'source_path': str(image_path), 'landmark_path': stored_path, 'detection_success': metadata['detection_success'], 'extraction_method': metadata['extraction_method'], 'handedness': metadata['handedness'], 'handedness_score': metadata['handedness_score'], 'image_width': metadata['image_width'], 'image_height': metadata['image_height'], 'crop_x1': metadata['crop_x1'], 'crop_y1': metadata['crop_y1'], 'crop_x2': metadata['crop_x2'], 'crop_y2': metadata['crop_y2'], 'error': metadata['error']})
        if index % 50 == 0 or index == len(rows):
            elapsed = time.perf_counter() - split_start
            print(f'[{index:4d}/{len(rows):4d}] full={full_count:4d} crop={crop_count:3d} failed={failure_count:3d} time={elapsed:.1f}s')
metadata_df = pd.DataFrame(records)
metadata_path = LANDMARK_ROOT / 'metadata.csv'
...
for split in ['train', 'val', 'test']:
    ...
    full = int((subset['extraction_method'] == 'full_image').sum())
    crop = int((subset['extraction_method'] == 'coco_crop').sum())
    print(f'{split:>5}: {len(subset):4d} total | {successful:4d} detected | {failed:3d} failed | {full:4d} full | {crop:3d} crop')
print()
print(f'Total images:   {len(metadata_df)}')
print(f"Successful:     {int(metadata_df['detection_success'].sum())}")
print(f"Failed:         {int((~metadata_df['detection_success']).sum())}")
print(f"Full-image:     {int((metadata_df['extraction_method'] == 'full_image').sum())}")
print(f"COCO-crop:      {int((metadata_df['extraction_method'] == 'coco_crop').sum())}")
...
display(pd.crosstab(metadata_df['split'], metadata_df['extraction_method']))
